...
const Charts = (function() {
    ...
    function getMeanAcrossRuns(simData, scenarioId) {
        const sr = simData.results[scenarioId];
        if (!sr || !sr.runs || sr.runs.length === 0) return [];
        if (sr.runs.length === 1) return sr.runs[0];
        return Statistics.computeMeanAcrossRuns(sr.runs);
    }
    ...
    function renderBlockTimeComparison(simData, selectedScenarios) {
        ...
        for (const s of selectedScenarios) {
            const run = getMeanAcrossRuns(simData, s.id);
            const points = run.map(r => ({ x: r.height, y: r.simMainChainBT }));
            datasets.push({
                label: s.label + (simData.results[s.id].numRuns > 1 ? ` (mean of ${simData.results[s.id].numRuns})` : ''),
        ...
        get('chartBlockTimeCompare', {
            ...
            options: {
                responsive: true, maintainAspectRatio: false,
                plugins: { title: { display: true, text: 'Block Time Comparison (Mean Across Runs)', color: '#eee', font: { size: 14 } }, legend: { labels: { color: '#ccc' } }, tooltip: { mode: 'index', intersect: false, callbacks: { label: (ctx) => `${ctx.dataset.label}: ${fmtDuration(ctx.parsed.y)}` } } },
                scales: makeScales('Block Time (s)'),
            },
        });
    }
